// Block-level diff of two ADF documents: added / removed / changed / moved.
// Identity comes from content hashes (parse.js); edited blocks re-pair via a
// similarity pass so an edit reads as one 'changed', not remove+add noise.
import { parse, contentOf } from './parse.js';
import { labelOf } from './render.js';

/**
 * Diff two ADF docs (or pre-parsed block arrays).
 * Returns ops: { op: 'added'|'removed'|'changed'|'moved', type, label,
 *                before?, after?, summary }.
 */
export function diff(a, b) {
  const blocksA = Array.isArray(a) ? a : parse(a);
  const blocksB = Array.isArray(b) ? b : parse(b);
